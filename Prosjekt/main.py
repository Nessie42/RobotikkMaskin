...
#image processing:
image = image_processing.gray_and_blur('face.jpg', [3,3])

contours_edge, contours_thresh, image_contours= image_processing.vectorize_image(image)

simple_vectors, simple_vector_image = image_processing.simplifying_vectors(image, contours_thresh, 0.01)

image_processing.image_for_tuning(simple_vector_image)
print(f'Simple vector array: {simple_vectors}')
...
for i in range(2):
    for row, value in enumerate(translated_array):
            value[0] = (px_array[row,0,0]/total_pixels_x)*A4X
            value[1] = (px_array[row,0,1]/total_pixels_y)*A4Y
